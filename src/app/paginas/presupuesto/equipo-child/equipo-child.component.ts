...
export class EquipoChildComponent implements OnInit {
  ...
  eliminarMaterial(material: MaterialEquipoArea, equipo: EquipoArea) {
    this.materialEquipoAreaHttp.eliminar(material.id).subscribe(() => {
  ...
  totalMateriales(equipo: EquipoArea) {
    ...
    equipo.materiales.forEach((material) => {
      total +=
        material.precio * material.cantidad * material.porcentajeGanancia;
    });
    return total;
  }
  ...
  actualizarTotalAsyncMaterial(
    material: MaterialEquipoArea,
  ...
  // un material
  actualizarTotalesEquipo(
    equipoArea: EquipoArea,
    conservar_total_equipo: boolean,
    conservar_total_material: boolean
  ) {
    let total_materiales = 0;
    equipoArea.materiales.forEach((material) => {
      total_materiales +=
        material.precio * material.cantidad * material.porcentajeGanancia * 1;
    });
    if (
    ...
    if (
      equipoArea.precioMaterialesEquipo > total_materiales &&
      !conservar_total_material
    ) {
      equipoArea.precioMaterialesEquipo = total_materiales;
    }
    equipoArea.total =
      equipoArea.precioEquipo * equipoArea.porcentajeGanancia +
    ...
    if (
      equipoArea.precioTotalPersonalizado > equipoArea.total &&
      !conservar_total_equipo
    ) {
      equipoArea.precioTotalPersonalizado = equipoArea.total;
    }
    equipoArea.costoBTU =
      equipoArea.total / (equipoArea.equipo.capacidad / 12000); // 12,000 BTU = 1 T
  }
